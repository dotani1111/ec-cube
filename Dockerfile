...
RUN find ${APACHE_DOCUMENT_ROOT} \( -path ${APACHE_DOCUMENT_ROOT}/vendor -prune \) -o -print0 \
  | xargs -0 chown -h www-data:www-data \
  && find ${APACHE_DOCUMENT_ROOT} \( -path ${APACHE_DOCUMENT_ROOT}/vendor -prune \) -o \( -type d -print0 \) \
  | xargs -0 chmod g+s \
  ;
